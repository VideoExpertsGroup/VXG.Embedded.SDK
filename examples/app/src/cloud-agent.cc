...
public:
    //! [Camera session registration on the Cloud callback]
    ...
    virtual void on_bye(proto::bye_reason reason) override {
        if (reason != proto::bye_reason::BR_RECONNECT)
            vxg::logger::error("Error {}", json(reason));
    }
    //! [Bye command callback]
    ...
    //! [Get audio detection config and capabilities]
...
public:
    my_media_stream(std::string url, std::string name)
        ...
        set_on_error_cb([&](const vxg::media::stream_error::report& report) {
            using namespace vxg::media;

            vxg::logger::instance(std::string("my-media-stream-") + name)
                ->error("Error {} from {} received", report.code, report.from);
            // switch (report.from)
            //  case : stream_error::origin::O_SOURCE - RTSP input stream error
            //  case : stream_error::origin::O_SINK - Cloud streaming error
        });
    }

    virtual bool get_supported_stream(
        proto::supported_stream_config& config) override {

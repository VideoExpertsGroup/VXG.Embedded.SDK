...
static int async_write(URLContext *h, const unsigned char *buf, int size) {
    Context         *c = h->priv_data;
    if (!c || !c->inner || !c->inner->prot || !c->inner->prot->url_write)
        return -1;
    return c->inner->prot->url_write(c->inner, buf, size);
}
...
static int async_get_file_handle(URLContext *h) {
    Context         *c = h->priv_data;
    if (!c || !c->inner || !c->inner->prot || !c->inner->prot->url_get_file_handle)
        return -1;
    return c->inner->prot->url_get_file_handle(c->inner);
}
...
const URLProtocol ff_async_protocol = {
    ...
    .url_read            = async_read,
    .url_write           = async_write,
    .url_seek            = async_seek,
    .url_close           = async_close,
    .url_get_file_handle = async_get_file_handle,

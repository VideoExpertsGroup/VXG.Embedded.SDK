...
static int rtsp_read_play(AVFormatContext *s)
{
    ...
    if (!(rt->server_type == RTSP_SERVER_REAL && rt->need_subscription)) {
        ...
        if (rt->state == RTSP_STATE_PAUSED) {
            ...
            /* Add 'Range:' only if there is no Range: in play_headers */
            if ((rt->play_headers && !av_stristr(rt->play_headers, "Range:")) || !rt->play_headers)
        ...
        if (rt->play_scale != 0.0) {
            char scale_str[32];
            snprintf(scale_str, sizeof(scale_str), "Scale: %.6f\r\n",
                     rt->play_scale);
            av_strlcat(cmd, scale_str, sizeof(cmd) - strlen(cmd) - 1);
        }

        if (rt->play_rate_control != -1) {
            char rate_control_str[32];
            snprintf(rate_control_str, sizeof(rate_control_str), "Rate-Control: %s\r\n",
                     rt->play_rate_control == 1 ? "yes" : "no");
            av_strlcat(cmd, rate_control_str, sizeof(cmd) - strlen(cmd) - 1);
        }

        if (rt->play_headers && strlen(rt->play_headers) > 2) {
            char tmp[1024];
            int hdr_len = strlen(rt->play_headers);

            strncat(cmd, rt->play_headers, sizeof(cmd) - strlen(cmd) - 1);
            if (rt->play_headers[hdr_len - 2] != '\r' ||
                rt->play_headers[hdr_len - 1] != '\n') {

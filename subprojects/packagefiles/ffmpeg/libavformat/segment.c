...
typedef struct SegmentContext {
    ...
    int   segment_save_cur_idx;   ///< write current segment filename to separate file
                           ///< filename 'list' + '.idx'
    ...
    int use_timeftime;     ///< flag to expand filename with timecode
    int64_t start_time;
...
static int set_segment_filename(AVFormatContext *s)
{
    ...
    if (seg->use_timeftime) {
        size_t len = strlen(buf);
        /* prepend filename with '.' as temporary file flag */
        if (sizeof(buf) >= len + 1) {
            char *p = strrchr(buf, '/');
            if (p) {
                memmove(p + 1, p, len + 1);
...
static int segment_list_write_current_entry(AVFormatContext* s)
{
    SegmentContext* seg = s->priv_data;
    ...
    snprintf(index_filename, sizeof(index_filename), "%s.idx", seg->list);
    ret = s->io_open(s, &idx_pb, index_filename, AVIO_FLAG_WRITE, NULL);
    if (ret < 0) {
        av_log(s, AV_LOG_ERROR, "Failed to open segment index '%s'\n",
               index_filename);
        return ret;
    }

    print_csv_escaped_str(idx_pb, seg->cur_entry.filename);
    avio_printf(idx_pb, ",%f,%f\n", seg->cur_entry.start_time,
                seg->cur_entry.end_time);

    ff_format_io_close(s, &idx_pb);
...
static int segment_start(AVFormatContext *s, int write_header)
{
    ...
    if (seg->segment_save_cur_idx)
        segment_list_write_current_entry(s);
...
static int segment_end(AVFormatContext *s, int write_trailer, int is_last)
{
    ...
    if (seg->use_timeftime) {
        char str_timecode[4096];
        char path[2048];
        double offset = (double)seg->start_time / (double)1000000;

        /* rename temp file */
        strncpy(path, seg->avf->url, sizeof(path));
        snprintf(str_timecode, sizeof str_timecode, "%s/%f_%f%s",
                 av_dirname(path), seg->cur_entry.start_time + offset,
                 seg->cur_entry.end_time + offset, strrchr(oc->url, '.'));
        av_log(s, AV_LOG_INFO, "Rename segment to %s\n", str_timecode);
        ff_rename(seg->avf->url, str_timecode, oc->pb);
    }
...
calc_times:
    if (seg->times) {
        ...
        if (seg->use_clocktime) {
            ...
            end_pts = s->output_ts_offset + seg->time * (seg->segment_count + 1);
        }
    }
...
static const AVOption options[] = {
    ...
    { "segment_save_cur_idx", "save currently writing segment info to file", OFFSET(segment_save_cur_idx), AV_OPT_TYPE_BOOL, {.i64 = 0 }, 0, 1, E },
    ...
    { "timecode_fname",    "set filename expansion with timecode", OFFSET(use_timeftime), AV_OPT_TYPE_BOOL, {.i64 = 0 }, 0, 1, E },

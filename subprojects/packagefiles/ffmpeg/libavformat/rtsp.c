...
const AVOption ff_rtsp_options[] = {
    ...
    { "async_tcp", "Async TCP(receiver runs in separate thread and writes to ringbuffer)", 0, AV_OPT_TYPE_CONST, {.i64 = (1 << RTSP_LOWER_TRANSPORT_ASYNC_TCP )}, 0, 0, DEC, "rtsp_transport" },
    ...
    { "play_rate_control", "Rate-Control PLAY header (-1 is for not adding the header, 0 is for Rate-Control: off, 1 is for Rate-Control: on", OFFSET(play_rate_control), AV_OPT_TYPE_INT, {.i64 = -1}, -1, 1, DEC|ENC },
    { "play_scale", "Scale PLAY header(0.0 is for not adding the header, -64.0..64.0 adding Scale header", OFFSET(play_scale), AV_OPT_TYPE_FLOAT, {.dbl = 0.0f}, -64.0f, 64.0f, DEC|ENC },
    { "play_headers", "Append RTSP PLAY headers", OFFSET(play_headers), AV_OPT_TYPE_STRING, {.str = NULL}, 0, 0, DEC },
#if FF_API_OLD_RTSP_OPTIONS
    ...
    case 'a':
        if (av_strstart(p, "control:", &p)) {
            ...
            if (rtsp_st->stream_index >= 0) {
                ...
                if (0 && st->codecpar->codec_id != AV_CODEC_ID_H264) {
                    s1->skip_media = 1;

                    av_log(s, AV_LOG_INFO, "Skipping stream codec_id: %d", st->codecpar->codec_id);
...
start:
    ...
    for (;;) {
        ...
        *q = '\0';
        ...
        av_log(s, AV_LOG_VERBOSE, "line='%s'\n", buf);
                                            ...
                                            int send_content_length)
{
    ...
    av_log(s, AV_LOG_VERBOSE, "Sending:\n%s--\n", buf);
...
int ff_rtsp_connect(AVFormatContext *s)
{
    ...
    if (rt->lower_transport_mask & (1 << RTSP_LOWER_TRANSPORT_ASYNC_TCP)) {
        rt->lower_transport_mask = 1 << RTSP_LOWER_TRANSPORT_TCP;
        lower_rtsp_proto = "async:tcp";
    }
...
redo:
    ...
    if (rt->transport == RTSP_TRANSPORT_RDT) {
        ...
        /* Provide absolute timestamp for caller */
        {
            RTPDemuxContext *rtpctx = rtsp_st->transport_priv;
            s->last_absolute_timestamp = av_rescale (rtpctx->last_rtcp_ntp_time - (NTP_OFFSET << 32), 1000000, 1LL << 32);

            if (rtpctx->st) {
                // FIXME: check what happen in case of rtptime wrapout
                s->last_absolute_timestamp +=
                    av_rescale (pkt->pts - rtpctx->last_rtcp_timestamp,
                                (uint64_t) rtpctx->st->time_base.num * 1000000,
                                            rtpctx->st->time_base.den);
            }
        }

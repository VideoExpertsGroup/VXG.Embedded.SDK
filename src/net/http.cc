...
static void _lws_log_emit_cx(struct lws_log_cx* cx,
                             int level,
                             const char* line,
                             size_t len) {
    vxg::logger* l = (vxg::logger*)(cx->stg);

    l->error("{}", line);
}

static lws_log_cx_t _http_log_cx;
#endif

#define HTTP_SEND_BUFFER_SIZE (32 * 1024)
#define HTTP_RECV_BUFFER_SIZE (32 * 1024)

#define MAX_HTTP_URL_LEN (4 * 4096)

bool http::_do_request(request_ptr req) {
    struct lws_client_connect_info i;
    const char *prot = nullptr, *address = nullptr, *path = nullptr;
    int port;
    char p[MAX_HTTP_URL_LEN];

    if (static_cast<size_t>(
            lws_snprintf(p, sizeof(p), "%s", (char*)req->url_.c_str())) !=
        req->url_.size()) {
...
#endif
        {
            logger->warn("HTTPS client accepts insecure SSL cert!");
    ...
    switch (reason) {
        ...
        case LWS_CALLBACK_CLIENT_CONNECTION_ERROR:
            logger->error("CLIENT_CONNECTION_ERROR: {}",
                          in ? (char*)in : "(null)");
            logger->error("Auth type: {}\nusername:{:a}\npassword:{:a}",
                          _req->auth_context_.auth_type_string(),
                          spdlog::to_hex(_req->auth_context_.username),
                          spdlog::to_hex(_req->auth_context_.password));

            _req->stats_finalize();

            if (_req->response_) {
                _req->error_reported_ = true;
                _req->response_(_req->response_body_, _req->status_);
            }
                ...
                if ((!_http->requests_q_.empty())) {
                    ...
                    while (!_http->requests_q_.empty()) {
                        ...
                        if (!_http->_do_request(_http->requests_q_.front())) {
                            ...
                            _http->requests_q_.front()->status_ = -2;
                            _http->requests_q_.front()->close_connection_ =
                                true;
                            if (_http->requests_q_.front()->response_ &&
                                !_http->requests_q_.front()->error_reported_)
                                _http->requests_q_.front()->response_(
                                    _http->requests_q_.front()->response_body_,
                                    _http->requests_q_.front()->status_);
                            else
                                logger->trace("Response cb is empty!");
...
#if LWS_LIBRARY_VERSION_NUMBER >= 4003000
    // lws logging ctx
    _http_log_cx.lll_flags = LLLF_LOG_CONTEXT_AWARE | LLL_ERR | LLL_WARN;
    _http_log_cx.u.emit_cx = _lws_log_emit_cx;
    _http_log_cx.opaque = (void*)logger.get();
    info.log_cx = &_http_log_cx;
#endif

 ...
 *
 * $ ssh -p 2200 -i /usr/local/share/libwebsockets-test-server/lws-ssh-test-keys
 * anyuser@127.0.0.1
 *
 ...
 */
#define TEST_SERVER_KEY_PATH "/tmp/lws-test-sshd-server-key"
...
static int ssh_ops_channel_create(struct lws* wsi, void** _priv) {
...
static int ssh_ops_channel_destroy(void* _priv) {
...
static int ssh_ops_tx_waiting(void* _priv) {
    struct sshd_instance_priv* priv = _priv;
    int s = 0;
...
static size_t ssh_ops_tx(void* _priv, int stdch, uint8_t* buf, size_t len) {
    struct sshd_instance_priv* priv = _priv;
    struct lws_ring* r;
    struct lws* wsi;
    size_t n;
...
static int ssh_ops_rx(void* _priv,
                      struct lws* wsi,
                      const uint8_t* buf,
                      uint32_t len) {
    struct sshd_instance_priv* priv = _priv;
...
static size_t ssh_ops_get_server_key(struct lws* wsi,
                                     uint8_t* buf,
                                     size_t len) {
...
static size_t ssh_ops_set_server_key(struct lws* wsi,
                                     uint8_t* buf,
                                     size_t len) {
...
static int ssh_ops_is_pubkey_authorized(const char* username,
                                        const char* type,
                                        const uint8_t* peer,
                                        int peer_len) {
    char *aps, *p, *ps;
...
static int ssh_cgi_env_add(struct sshd_instance_priv* priv,
                           const char* name,
                           const char* value) {
...
static int ssh_ops_set_env(void* _priv, const char* name, const char* value) {
    struct sshd_instance_priv* priv = _priv;

    return ssh_cgi_env_add(priv, name, value);
}

static int ssh_ops_pty_req(void* _priv, struct lws_ssh_pty* pty) {
    struct sshd_instance_priv* priv = _priv;
...
static int ssh_ops_child_process_io(void* _priv,
                                    struct lws* wsi,
...
static int ssh_ops_child_process_terminated(void* priv, struct lws* wsi) {
    lwsl_notice("%s\n", __func__);
    return -1;
}

static int ssh_ops_exec(void* _priv,
                        struct lws* wsi,
                        const char* command,
                        lws_ssh_finish_exec finish,
                        void* finish_handle) {
...
static int ssh_ops_shell(void* _priv,
                         struct lws* wsi,
                         lws_ssh_finish_exec finish,
                         void* finish_handle) {
    struct sshd_instance_priv* priv = _priv;
    const char* cmd[] = {"/bin/bash", "-i", "-l", NULL};
    lwsl_notice("%s: SHELL\n", __func__);
...
static size_t ssh_ops_banner(char* buf,
                             size_t max_len,
                             char* lang,
                             size_t max_lang_len) {
    int n = lws_snprintf(
...
static void ssh_ops_disconnect_reason(uint32_t reason,
                                      const char* desc,
...
static const struct lws_protocol_vhost_options
    pvo_ssh_ops = {NULL, NULL, "ops", (void*)&ssh_ops},
    pvo_ssh_base =
        {
            NULL, &pvo_ssh_ops, "lws-ssh-base",
            "" /* ignored, just matches the protocol name above */
},
    pvo_ssh = {
        &pvo_ssh_base, NULL, "lws-sshd-demo1",
        "" /* ignored, just matches the protocol name above */
};
...
static int callback_lws_sshd_demo1(struct lws* wsi,
                                   enum lws_callback_reasons reason,
                                   void* user,
                                   void* in,
                                   size_t len) {
    struct per_vhost_data__lws_sshd_demo* vhd =
        (struct per_vhost_data__lws_sshd_demo*)lws_protocol_vh_priv_get(
    ...
    switch (reason) {
        ...
        default:
            ...
            if (!vhd->ssh_base_protocol) {
                ...
                if (vhd->ssh_base_protocol)
                    user = lws_adjust_protocol_psds(
                        wsi, vhd->ssh_base_protocol->per_session_data_size);
            }

            if (vhd->ssh_base_protocol)
                return vhd->ssh_base_protocol->callback(wsi, reason, user, in,
                                                        len);
            else
...
const struct lws_protocols lws_sshd_demo_protocols1[] = {
    {"lws-sshd-demo1", callback_lws_sshd_demo1, 0,
...
struct lws_context* lws_create_sshd_context() {

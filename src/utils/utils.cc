...
std::string now_ISO8601_UTC_packed() {
    return to_iso_8601_packed(utils::time::now());
}

std::string now_ISO8601_UTC() {
    return to_iso_8601(utils::time::now());
}
...
std::string to_iso(cloud::time t) {
    ...
    return date::format("%FT%TZ", date::floor<cloud::duration>(t));
}

std::string to_iso2(cloud::time t) {
    using namespace std::chrono;
    return date::format("%FT%T", date::floor<cloud::duration>(t));
}
...
cloud::time from_double(double t) {
    ...
    return std::chrono::time_point<system_clock, cloud::duration>(
        duration_cast<time_spec::precision>(std::chrono::duration<double>(t)));
}
...
double to_double(cloud::time t) {
    ...
    std::chrono::time_point<system_clock, ds> result = t;

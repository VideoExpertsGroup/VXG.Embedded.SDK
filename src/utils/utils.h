...
struct uri {
    ...
    static bool parse(const std::string& uri) {
        ...
        const char* USER_REGEX = "(?:([^:\\s]+):)?";
        // const char* PASSWORD_REGEX = "(?:([^@/\\s:]+)@)?";
        // mandatory. match anything other than @ / : or whitespace
        const char* HOST_REGEX = "([^@\\s:]+)?";
        // after the : match 1 to 5 digits
        const char* PORT_REGEX = "(?::([0-9]{1,5}))?";
        // after the / match anything other than : # ? or whitespace
        const char* PATH_REGEX = "(//[^:#?\\s]*)?";
        // after the ? match any number of x=y pairs, seperated by & or ;
        const char* QUERY_REGEX = "(\?(?:&?(?:[^&=](?:=[^?=&])?)*)+)?";
        // after the # match anything other than # or whitespace
        const char* FRAGMENT_REGEX = "(#([^#\\s]*))?";

        static const std::regex regExpr(
            std::string("^") + SCHEME_REGEX + USER_REGEX + HOST_REGEX +
            PORT_REGEX + PATH_REGEX + QUERY_REGEX + FRAGMENT_REGEX + "$");

        std::smatch matchResults;
        if (std::regex_match(uri.cbegin(), uri.cend(), matchResults, regExpr)) {
            scheme.assign(matchResults[0].first, matchResults[0].second);
            user.assign(matchResults[1].first, matchResults[1].second);
            host.assign(matchResults[2].first, matchResults[2].second);
            port.assign(matchResults[3].first, matchResults[3].second);
            path.assign(matchResults[4].first, matchResults[4].second);
            query.assign(matchResults[5].first, matchResults[5].second);
            fragment.assign(matchResults[6].first, matchResults[6].second);

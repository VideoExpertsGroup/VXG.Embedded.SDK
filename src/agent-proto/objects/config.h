...
public:
    ...
    std::string cam_base_uri(bool secure = true,
                             const std::string& input_host = "") {
        std::string proto = secure ? "wss" : "ws";
        int port = secure ? cam_sp : cam_p;
        auto host = input_host.empty() ? cam : input_host;

        return utils::string_format("%s://%s:%d", proto.c_str(), host.c_str(),
                                    port);
    }

    std::string pack() { return utils::base64_encode(json(*this).dump()); }

    static access_token parse(std::string packed_token) {
        access_token result;
        try {
            std::string decoded_token = utils::base64_decode(packed_token);
            result = json::parse(decoded_token);
            __normalize_access_token(result);

...
NLOHMANN_JSON_SERIALIZE_ENUM( stream_reason, {
    ...
    {SR_SERVER_BY_EVENT, "server_by_event"},
...
struct event_caps {
    ...
    //! @internal
    //! Event statefulness flag, indicates if event can be active during time
    //! period.
    //!
    //! For every stateful event user code should generate 2 events with

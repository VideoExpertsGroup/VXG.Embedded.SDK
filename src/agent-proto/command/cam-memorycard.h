...
//! 3.43 cam_memorycard_recording (SRV)
//! Server informs the camera about the parameters of continuous recording to
//! the memory card.
struct cam_memorycard_recording : public base_command {
    typedef std::shared_ptr<cam_memorycard_recording> ptr;

    cam_memorycard_recording() { cmd = CAM_MEMORYCARD_RECORDING; }
    virtual ~cam_memorycard_recording() {}

    //! string, identifies the request. From get_cam_memorycard_timeline.
    std::string stream_id {UnsetString};
    alter_bool enabled {alter_bool::B_INVALID};

    JSON_DEFINE_DERIVED_TYPE_INTRUSIVE(cam_memorycard_recording,
                                       base_command,
                                       stream_id,
                                       enabled);
};

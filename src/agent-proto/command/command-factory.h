...
public:
    static std::shared_ptr<base_command> parse(const json& j) {
        ...
        if (j.contains("cmd")) {
            ...
            else if (cmd == SET_AUDIO_DETECTION)
                command = std::shared_ptr<set_audio_detection>(
                    new set_audio_detection(j));
            else if (cmd == AUDIO_DETECTION_CONF)
                command = std::shared_ptr<audio_detection_conf>(
                    new audio_detection_conf(j));
    ...
    static base_command::ptr reply(const base_command* orig_cmd,
        ...
        if (!cmd.empty() && cmd != UnsetString) {
            if (cmd == REGISTER)
                ...
                command = std::shared_ptr<set_audio_detection>(
                    new set_audio_detection());
            else if (cmd == AUDIO_DETECTION_CONF)
                command = std::shared_ptr<audio_detection_conf>(
                    new audio_detection_conf());
            else if (cmd == GET_CAM_AUDIO_CONF)
                command = std::shared_ptr<get_cam_audio_conf>(

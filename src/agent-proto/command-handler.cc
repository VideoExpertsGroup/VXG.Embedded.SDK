...
command_handler::command_handler(transport::worker::ptr transport)
...
bool command_handler::try_connect() {
    return transport_->connect(cm_config_.get_address());
}
...
json command_handler::handle_hello(hello* hello) {
    try {
        if (hello->ca != UnsetString)
            cm_config_.ca = hello->ca;
        if (hello->sid != UnsetString)
            profile::global::instance().cm_registration_sid = cm_config_.sid =
                hello->sid;
        // For instances with old proto
        if (hello->upload_url != UnsetString)
            cm_config_.upload_url = hello->upload_url;
        // New proto, overrides upload_url field if presented
        if (hello->upload_uri != UnsetString)
            cm_config_.upload_url = hello->upload_uri;
        if (hello->media_server != UnsetString)
            cm_config_.media_server = hello->media_server;
        if (hello->connid != UnsetString)
            cm_config_.conn_id = hello->connid;
...
json command_handler::handle_configure(configure* configure) {
    try {
        if (!__is_unset(configure->pwd))
            cm_config_.pwd = configure->pwd;

        if (!__is_unset(configure->uuid))
            cm_config_.uuid = configure->uuid;

        if (!__is_unset(configure->connid))
            cm_config_.conn_id = configure->connid;

        if (!__is_unset(configure->tz)) {
            cm_config_.timezone = configure->tz;
            on_set_timezone(cm_config_.timezone);
        }

        if (!__is_unset(configure->server))
            cm_config_.reconnect_server_address = configure->server;
...
json command_handler::handle_get_cam_status(get_cam_status* getCamStatus) {
    ...
    camStatus->ip = cm_config_.device_ipaddr;
    camStatus->activity = cm_config_.activity;
    camStatus->streaming = cm_config_.streaming;
...
json command_handler::handle_cam_hello(cam_hello* camHello) {
    ...
    if (!__is_unset(camHello->cam_id))
        cm_config_.cam_id = camHello->camid();

    if (!__is_unset(camHello->media_uri))
        cm_config_.media_server = camHello->media_uri;

    if (!__is_unset(camHello->path))
        cm_config_.cam_path = camHello->path;

    cm_config_.activity = camHello->activity;
...
json command_handler::handle_stream_start(stream_start* streamStart) {
    ...
    if (reason == proto::stream_reason::SR_RECORD &&
        streamStart->storage_direct_recording == alter_bool::B_TRUE)
        reason = proto::stream_reason::SR_RECORD_DIRECT;
...
json command_handler::handle_cam_update_preview(
    ...
    std::string url = cm_config_.buildUrl(manager_config::UT_UPLOAD_PREVIEW);
...
bool command_handler::do_register() {
    ...
    on_get_timezone(cm_config_.timezone);

    command->ver = cm_config_.agent_version;
    command->tz = cm_config_.timezone;
    command->vendor = cm_config_.device_vendor;

    if (!cm_config_.pwd.empty())
        command->pwd = cm_config_.pwd;

    if (!cm_config_.sid.empty())
        command->prev_sid = cm_config_.sid;
    else if (!profile::global::instance().cm_registration_sid.empty())
        command->prev_sid = profile::global::instance().cm_registration_sid;

    if (!cm_config_.reg_token.isEmpty())
        command->reg_token = cm_config_.reg_token.getToken();
...
bool command_handler::do_cam_register() {
    if (!registered_) {
        ...
        command->ip = cm_config_.device_ipaddr;
        command->uuid = cm_config_.uuid;
        command->brand = cm_config_.device_brand;
        command->model = cm_config_.device_model;
        command->sn = cm_config_.device_serial;
        command->raw_messaging = profile::global::instance().raw_messaging;
        command->version = cm_config_.device_fw_version;
        command->type = cm_config_.device_type;

        on_registered(cm_config_.sid);
...
void command_handler::_flush_command_acks() {
    for (auto ack = acks_.begin(); ack != acks_.end();) {
        ...
        if (ack->second.ack_cb_)
            ack->second.ack_cb_(true, nullptr);
    }
}
...
void command_handler::handleMessage(transport::Message& message) {
    ...
    if (!__is_unset(command->camid()) && cm_config_.cam_id &&
        command->camid() != cm_config_.cam_id) {
...
bool command_handler::send_command_wait_ack(
    const json& message,
    std::function<void(bool timed_out,
                       proto::command::base_command::ptr ack_cmd)> ack_cb,

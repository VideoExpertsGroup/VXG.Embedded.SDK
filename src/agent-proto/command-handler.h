...
protected:
    ...
    using ack_cb = std::function<void(bool timed_out,
                                      proto::command::base_command::ptr ack_cmd,
                                      std::shared_ptr<void> ack_opaque)>;
    struct command_ack {
        ack_cb ack_cb_;
...
public:
    command_handler(const agent::config& config,
                    const proto::access_token& access_token,
                    transport::worker::ptr transport = nullptr);
...
protected:
    ...
    json handle_set_audio_detection(set_audio_detection* setAudioDetection);
...
protected:
    ...
    bool send_command_wait_ack(const json& message,
                               ack_cb ack_cb,
    ...
    virtual bool on_set_audio_detection(
        const proto::audio_detection_config& conf) = 0;
    virtual bool on_get_audio_detection(
        proto::audio_detection_config& conf) = 0;

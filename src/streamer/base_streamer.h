...
//! @brief On error callback, used for both ISink and ISource if was provided
//!        by user.
...
public:
    ...
    //! Method may be overriden, default implementation calls on_error_cb that
    //! was provided by user with set_error_cb().
    ...
    //! @brief Processed stream duration
    //! @return duration
    virtual cloud::duration duration() { return std::chrono::seconds(0); };
    ...
    void set_error_cb(on_error_cb cb) { on_error_cb_ = cb; }
...
private:
    ...
    bool eos_ {false};
    std::function<void(cloud::duration)> eos_cb_ {nullptr};
};
...
public:
    ...
    //! @private
    bool unlinkSink(ISink::ptr sink) {
#if __cplusplus <= 199711L
        pthread_mutex_lock(&multiqueue_lock_);
#else
        const std::lock_guard<std::mutex> lock(multiqueue_lock_);
...
private:
    static void __update_sink_queue_stats(ISink::ptr sink) {
        if (sink->name() == "rtmp-sink")
    ...
    void _errorAll(StreamError error) {
        ...
        // use copy here
        std::map<ISink::ptr, std::shared_ptr<MediaFrameQueue>> multiqueue_copy =
            sink_queue_map_;
...
private:
    bool running_;
    std::map<ISink::ptr, std::shared_ptr<MediaFrameQueue>> sink_queue_map_;
#if __cplusplus <= 199711L
    pthread_t thr_;

...
class mp4_recorder_sink : public vxg::media::ffmpeg::multifile_sink {
    ...
    void _on_new_recroding(std::string filepath,
                           cloud::time begin,
                           std::chrono::milliseconds duration) {
        using namespace std::experimental::filesystem;
    ...
    static Streamer::ISource::ptr _check_source(Streamer::ISource::ptr source) {
        if (!source)
            return std::make_shared<vxg::media::rtsp_source>(
                vxg::media::rtsp_source::TCP, _rtsp_medias_filter());
        return source;
    }
...
public:
    rtsp_segmented_mp4_recording_stream()
        : vxg::media::stream("mp4-segmented-recording",
                             std::make_shared<vxg::media::rtsp_source>(
                                 vxg::media::rtsp_source::TCP,
                                 _rtsp_medias_filter()),

...
class rtsp_source : public ffmpeg::Source {
    ...
    const int RTSP_TIMEOUT_SEC = 5;
    const int RTSP_TIMEOUT_UDP = RTSP_TIMEOUT_SEC * 1000;
    const int RTSP_TIMEOUT_TCP = RTSP_TIMEOUT_SEC * 1000000;
...
public:
    ...
    rtsp_source(bool rtp_over_tcp = true,
        ...
        : rtp_over_tcp_ {rtp_over_tcp}, media_types_ {media_types} {}
    ...
    rtsp_source(std::string rtp_transport = "tcp",
                std::vector<Streamer::MediaType> media_types =
                    std::vector<Streamer::MediaType>(0))
        : rtp_transport_ {rtp_transport}, media_types_ {media_types} {}
    ...
    virtual bool init(std::string url) {
        ...
        int timeout =
            (std::strstr(rtp_transport_.c_str(), "udp") ? RTSP_TIMEOUT_UDP
                                                        : RTSP_TIMEOUT_TCP);

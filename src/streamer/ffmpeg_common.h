...
private:
    ...
    void _check_fix_ts(AVPacket* pkt) {
        // Trying to fix the timestamps if necessary
        auto stream = ff_format_ctx_->streams[pkt->stream_index];
        auto max = stream->cur_dts +
        ...
        if (pkt->dts != AV_NOPTS_VALUE && stream->cur_dts != AV_NOPTS_VALUE &&
            pkt->dts < max) {
            logger->warn(
                "Non-monotonic DTS for in stream {}; previous: {} current "
                "{}, changing to {}.",
                pkt->stream_index, stream->cur_dts, pkt->dts, max);
            if (pkt->pts >= pkt->dts)

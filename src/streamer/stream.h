...
//!
class stream
    : public cloud::utils::queued_async_handler<Streamer::StreamError> {
    vxg::logger::logger_ptr logger {vxg::logger::instance("base-media-stream")};

    void on_error(std::string source, Streamer::StreamError e) {
        if (source == "sink")
            stop_sink();
        if (source == "source") {
            stop_sink();
...
protected:
    Streamer::on_error_cb on_error_cb_ {[](Streamer::StreamError e) -> void {}};
...
public:
    ...
    stream(std::string name,
        ...
        : cloud::utils::queued_async_handler<Streamer::StreamError>(),
          name_ {name},
          ...
          source_ {source},
          sink_ {sink} {
        source_->set_error_cb(
            [&](Streamer::StreamError e) { on_error("source", e); });
        sink_->set_error_cb(
            [&](Streamer::StreamError e) { on_error("sink", e); });
    }
    virtual ~stream() {}
    ...
    //! @private
    virtual void set_on_error_cb(Streamer::on_error_cb e_cb) {
        on_error_cb_ = e_cb;
        set_handler(e_cb);
    }

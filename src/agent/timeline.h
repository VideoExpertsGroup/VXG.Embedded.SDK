...
public:
    ...
    struct item : public period {
        ...
        void set_data(std::vector<uint8_t> data) { data = data; }
...
class vfs_storage : public timed_storage {
    ...
    bool _period_from_filename(std::string filename, period& p) {
        ...
        if (timestamps.size() == 2) {
            p.begin = utils::time::from_iso(timestamps[0]);
            p.end = utils::time::from_iso(timestamps[1]);
        }

        if (p.begin == utils::time::null() || p.end == utils::time::null()) {
    ...
    bool _filename_from_period(period p, std::string& filename) {
        ...
        filename = fs::path(to_iso(p.begin) + "_" + to_iso(p.end));
...
class cloud_storage : public timed_storage {
    ...
    vxg::cloud::agent::proto::access_token::ptr token_;
    vxg::cloud::transport::libwebsockets::http::ptr http_;
    bool secure_transport_ {true};

public:
    cloud_storage(agent::proto::access_token::ptr token,
...
public:
    timeline(vxg::cloud::agent::proto::access_token::ptr access_token,
             transport::libwebsockets::http::ptr http = nullptr)

...
public:
    ...
    //! @brief Create uploader object pointer
    //!
    //! @param access_token VXG Cloud Access token used to retrieve already
    //! uploaded data
    //! @param http HTTP transport to perform HTTP request to the VXG Cloud API
    //! User is responsible for starting/stopping of @p http object
    //! If nullptr uploader will allocate its own transport.
    //! @param direct_upload_cb Callback called to upload data that is ready
    //! @param send_command_cb Callback for sending arbitary VXG Cloud commands
    //! @return ptr to uploader object
    //!

...
public:
    ...
    rtsp_stream(
        ...
        : agent::media::stream(
              name,
              // RTSP source
              std::make_shared<vxg::media::rtsp_source>(transport,
                                                        _rtsp_medias_filter()),
              recorder_needs_source),
          source_uri_ {source_url} {}

    //! @brief Construct a new rtsp stream object using provided rtsp_src
    //!
    //! @param source_url RTSP url
    //! @param name Unique stream name
    //! @param rtsp_src rtsp_source object pointer

...
struct mock_uploader : public agent::segmented_uploader {
    ...
    static proto::access_token __gen_dummy_token() {
        return proto::access_token();
    }
...
public:
    ...
    virtual void SetUp() {
        ...
        ON_CALL(*std::dynamic_pointer_cast<mock_media_stream>(this->stream_),
                record_get_list(_, _, _))
            .WillByDefault(Invoke(std::bind(
                &uploader_test::__get_records_list, this, placeholders::_1,
                placeholders::_2, placeholders::_3)));
        EXPECT_CALL(
            *std::dynamic_pointer_cast<mock_media_stream>(this->stream_),
            record_get_list(_, _, _))
            .Times(AnyNumber());

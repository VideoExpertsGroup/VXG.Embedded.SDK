...
void TEST_MULTI_OFFSETS(uploader_test* obj,
    ...
    obj->SCHED_UPLOAD_MULTIPLE(periods, pre, post);

    EXPECT_TRUE(obj->wait_all_segments_finished());
    ...
    for (size_t i = 0; i < std::min(a.size(), b.size()); i++) {
        vxg::logger::debug("Verify result: {} - {} ? {} - {}",
...
TEST_F(uploader_test, Bad_Segmenter) {
    ...
    s->realtime = true;
    s->last_proccessed_time = utils::time::now();
    s->delay = seconds(1);

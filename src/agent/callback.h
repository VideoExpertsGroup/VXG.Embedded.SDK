...
public:
    ...
    //! the profile::global::instance().cm_register_sid before the next
    //! vxg::cloud::agent::manager::start()
    ...
    //!
    //! @brief Get events configuration
    //!
    //! @param[out] config events config
    //! @return true if \p config is valid
    //! @return false if \p config is invalid
    //!
    ...
    //! @return true if firware upgrade was successfull.
    //! @return false if firware upgrade failed.
    ...
    //! \cond
    virtual void set_periodic_events(const char* name,
                                     int period,
                                     bool active) {}

    virtual bool get_event_limits(time_t& pre, time_t& post) {
        vxg::logger::warn("{} not implemented", __func__);
        return false;
    }
    virtual void set_event_limits(time_t pre, time_t post) {}
    //! \endcond
};
}  // namespace agent
}  // namespace cloud
